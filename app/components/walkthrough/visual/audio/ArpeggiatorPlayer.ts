...
export class ArpeggiatorPlayer {
    ...
    constructor() {
        ...
        // Ambient tail reverb
        this.ambientReverb = createReverb('deep');
        this.ambientReverb.wet.value = 1.0;
        this.ambientGain = new Tone.Gain(0.1).connect(this.limiter);
        this.ambientReverb.connect(this.ambientGain);

        // Main effects chain
        this.reverb = createReverb('spatial');
        ...
        // Chain: Filter -> Delay -> Reverb -> MasterGain
        // Synths will connect to Panner -> Gain -> [Filter input]
        this.filter.connect(this.delay);
        this.delay.connect(this.reverb);
        this.reverb.connect(this.masterGain);
        this.reverb.connect(this.ambientReverb);
    ...
    setPatternFromEngine(voices: EdgeVoicePattern[]) {
        ...
        for (let i = 0; i < this.MAX_VOICES; i++) {
            ...
            if (i < voices.length) {
                const v = voices[i];
                updatePannerPosition(voice.panner, v.position, 0);
                voice.currentNote = v.note;
                voice.isEdgeNode = v.isEdgeNode;
                voice.sequence.events = v.events;
                voice.gain.gain.rampTo(v.isEdgeNode ? 0.6 : 0.15, 0.5, now);
            } else {
                voice.sequence.events = [];
                voice.gain.gain.rampTo(0, 0.5, now);
            }
        }
        this.masterGain.gain.rampTo(0.5, this.FADE_IN_TIME, now);
