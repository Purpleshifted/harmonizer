...
export class ChordPlayer {
    ...
    readonly RE_TRIGGER_INTERVAL = 6000; // 6 seconds (Matches WaveEffect period)

    // Fade
    readonly FADE_TIME = 2.0; // Softer fade
    ...
    constructor(sharedSpatialReverb: Tone.Reverb, sharedDeepReverb: Tone.Reverb) {
        ...
        // Spatial Chain (Wet 0.4 -> Dry 0.6, Send 0.4)
        ...
        // Center Chain (Wet 0.8 -> Dry 0.2, Send 0.8)
        this.centerDry = new Tone.Gain(0.2).connect(this.masterGain);
        this.centerSend = new Tone.Gain(0.8).connect(this.deepReverb);
        this.vibratoDeep.connect(this.centerDry);
        this.vibratoDeep.connect(this.centerSend);

        // Astral Chain (Wet 0.8 -> Dry 0.2, Send 0.8) - No Vibrato
        this.astralDry = new Tone.Gain(0.2).connect(this.masterGain);
        this.astralSend = new Tone.Gain(0.8).connect(this.deepReverb);
        ...
        this.baseGain.connect(this.vibratoSpatial); // Connect to Spatial Chain

        // === 2. Spatial Layer (Horns) ===
        for (let i = 0; i < this.MAX_HORNS; i++) {
            this.hornPool.push(new HornVoice(this.vibratoSpatial)); // Connect to Spatial Chain
        }

        // === 3. Center Layer (Synth) ===
        this.centerGain = new Tone.Gain(0.3);
        // Removed internal centerReverb creation
        ...
        this.centerFilter.connect(this.centerGain);
        this.centerGain.connect(this.vibratoDeep); // Connect to Deep Chain

        // === 4. Astral Layer (Arp) ===
        this.astralGain = new Tone.Gain(1.0);
        // Removed internal astralReverb creation
        ...
        this.astralDelay.connect(this.astralGain);
        this.astralGain.connect(this.astralDry); // Connect to Astral Chain (Split manually)
        this.astralGain.connect(this.astralSend);
    ...
    playChord(notes: string[], positions: THREE.Vector3[], isMajor: boolean) {
        ...
        if (this.currentNotes.join('-') !== notesKey) {
            if (now - this.lastTriggerTime > 0.1) {
                ...
                this.updateAstralSequence(validNotes, isMajor);
    ...
    dispose() {
        ...
        // Shared reverb - do not dispose of passed reverb
